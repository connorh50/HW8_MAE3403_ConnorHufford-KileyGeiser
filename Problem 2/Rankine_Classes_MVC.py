...
class rankineModel():
    def __init__(self):
        ...
        #Initialize states as stateProps objects (i.e., stateProperties)
...
class rankineView():
    ...
    def updateUnits(self, Model=None):
        ...
        - Updates the GUI outputs using data from the provided model.
        - Converts and updates the pressure fields (PHigh and PLow) based on the active unit system.
        - Updates the Turbine Inlet Temperature (THigh) if the quality radio button is not selected.
        - Updates the labels to display the correct units for pressures and temperature.
        :param Model:  reference to the model containing settings, conversion factors, and steam properties
        :return: None
        """
        # Step 0: Update outputs on the GUI using current model data.
        self.outputToGUI(Model=Model)
        # Update units displayed on labels

        # Step 1: Update pressures for PHigh and PLow.
        # Determine the pressure conversion factor:
        # Use 1 if SI units are active; otherwise, convert from bar to psi using UC.bar_to_psi.
        pCF = 1 if Model.SI else UC.bar_to_psi  # pressure conversion factor
        try:
            # Retrieve and convert the PHigh input text to a float,
            # then update the field with the converted pressure value.
            pHigh = float(self.le_PHigh.text())
            self.le_PHigh.setText(f"{pHigh * pCF:.2f}")
        except ValueError:
            # If the input is invalid or empty, do not update PHigh.
            pass

        try:
            # Retrieve and convert the PLow input text to a float,
            # then update the field with the converted pressure value.
            pLow = float(self.le_PLow.text())
            self.le_PLow.setText(f"{pLow * pCF:.2f}")
        except ValueError:
            # If the input is invalid or empty, do not update PLow.
            pass

        # Step 2: Update the Turbine Inlet Temperature (THigh) if quality is not selected.
        if not self.rdo_Quality.isChecked():
            try:
                # Re-read the PHigh value and convert it to bar if necessary.
                pHigh = float(self.le_PHigh.text())
                pHigh_bar = pHigh if Model.SI else pHigh * UC.psi_to_bar
                # Obtain the saturation temperature (tsat) based on the pressure.
                tsat = Model.steam.getsatProps_p(pHigh_bar).tsat
                # Convert the saturation temperature to the proper display unit.
                tsat_display = tsat if Model.SI else UC.C_to_F(tsat)
                self.le_TurbineInletCondition.setText(f"{tsat_display:.2f}")
            except ValueError:
                # If conversion fails, clear the turbine inlet condition field.
                self.le_TurbineInletCondition.setText("")
            pass

        # Step 3: Update the labels to display the correct units.
        self.lbl_PHigh.setText("P High (bar)" if Model.SI else "P High (psi)")
        self.lbl_PLow.setText("P Low (bar)" if Model.SI else "P Low (psi)")
        x = self.rdo_Quality.isChecked()
        self.lbl_TurbineInletCondition.setText(
            "Turbine Inlet: {}{}".format('x' if x else 'THigh', '' if x else (' (C)' if Model.SI else ' (F)'))
        )
        pass
